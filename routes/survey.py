...
def submit_dass21():
    from models import SurveyRespondent, SurveyAnswer, Survey, QuestionOption, Question, QuestionCategory
    from datetime import datetime
    import json
    ...
    # Simpan jawaban
    for item in answers:
        option = QuestionOption.query.filter_by(id=item['option_id']).first()
    ...
    for cat in categories:
        cat_obj = QuestionCategory.query.filter_by(name=cat).first()
        if not cat_obj:
            results[cat] = {"score": 0, "label": "Tidak Diketahui"}
            continue

        questions = Question.query.filter_by(question_category_id=cat_obj.id).all()
        question_ids = [q.id for q in questions]

        jawabans = SurveyAnswer.query.filter(
            SurveyAnswer.survey_respondent_id == respondent.id,
            SurveyAnswer.question_id.in_(question_ids)
        ).all()

        score = sum([int(j.answer) for j in jawabans]) * 2  # DASS21 scoring
        label = categorize(score, cat)
        results[cat] = {"score": score, "label": label}
